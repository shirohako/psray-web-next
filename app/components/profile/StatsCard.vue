  ...
  <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <dl class="grid grid-cols-2 gap-y-4">
      <div>
        ...
        <dd class="mt-0.5 text-lg font-bold text-slate-900">{{ fmt(profile.games_played) }}</dd>
      </div>
      <div>
      ...
      <div>
        ...
        <dd class="mt-0.5 text-lg font-bold text-slate-900">{{ fmt(profile.completed_games) }}</dd>
      </div>
      <div>
        <dt class="inline-flex items-center gap-1 text-xs text-slate-400">
          <LucideIcon :icon="XCircle" class="size-3.5 text-rose-400" />已弃坑
        </dt>
        <dd class="mt-0.5 text-lg font-bold text-slate-900">{{ fmt(profile.abandoned_games) }}</dd>
      </div>
    </dl>
    ...
    <div class="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4">
      <div class="flex items-center gap-2">
        ...
        <div class="leading-tight">
          <div class="text-sm font-semibold text-slate-900">{{ fmt(profile.favorite_games) }}</div>
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="leading-tight">
          <div class="text-sm font-semibold text-slate-900">{{ fmt(profile.game_reviews) }}</div>
